...
import pandas as pd
import Gapp.models
from .models import dj,store,effective,Feadback,User
...
def feadback(request):
    if request.method == 'POST':
        ...
        unit = Feadback.objects.create(username=name, userEmail=email, userFeadback=feadback, userrecommend=recommend)
        unit.save()
