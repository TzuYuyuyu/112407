...
import pandas as pd
import Gapp.models
from .models import dj,store,effective,feedback,FavoriteLocation
...
def add_favorite_location(request):
    if request.method == 'POST':
        ...
        # 在这里将经纬度坐标存储到数据库
        favorite_location = FavoriteLocation.objects.create(latitude=latitude, longitude=longitude)
...
def show_favorite_locations(request):
    favorite_locations = FavoriteLocation.objects.all()
